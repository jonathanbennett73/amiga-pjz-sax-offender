...
REM Lossless
REM ffmpeg -i "output.avi" -vf "crop=704:544:44:20,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2" -c:a copy -c:v libx264 -crf 0 -preset veryfast -coder 1 -pix_fmt yuv420p -f matroska "output_lossless_50fps.mkv"
REM ffmpeg -i "output.avi" -vf "fps=fps=30,crop=704:544:44:20,scale=1920:1080:force_original_aspect_ratio=decrease,pad=1920:1080:(ow-iw)/2:(oh-ih)/2" -c:a copy -c:v libx264 -crf 0 -preset veryfast -coder 1 -pix_fmt yuv420p -f matroska "output_lossless_30fps.mkv"
